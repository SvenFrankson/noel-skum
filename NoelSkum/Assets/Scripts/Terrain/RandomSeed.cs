...
public class RandomSeed
{
    ...
    private float[] Rands
    {
        get
        {
            if (this.rands == null)
            {
                ...
                {
                    this.rands[i] = Random.Range(0f, 1f);
                }
            }
    ...
    public int RandRange(int i, int j, int d, int min, int max)
    {
        return Mathf.FloorToInt((Rand(i, j, d) + 1f) / 2f * (max - min + 1f) + min);
    }
    ...
    public float Rand(int i, int j, int d)
    {
        int index = Mathf.Abs(i * 11 + j * 13 + d);
        return Rands[index % RANDOMLENGTH];
    }
}
